...
var ConfigView = View.extend({
    SETTING_KEYS: [
        'sils.icon_cache_path'
    ],

    settingControlId: function (key) {
        return '#g-wt-sils-' + key.substring(5).replace(/_/g, '-');
    },

    events: {
        'submit .g-wt-sils-config-form': function (event) {
    ...
    _saveSettings: function (providerId, settings) {
        restRequest({
            ...
            this.$('#g-wt-sils-config-error-message').text(resp.responseJSON.message);
        }, this));
    }
});
